...
namespace PetsHome.UI.Controllers
{
    public class VacunaController : BaseController
    {
        ...
        public async Task<IActionResult> Add(VacunaViewModel model)
        {
            if (model.vac_Id == 0)
            {
                bool createdItem = await _vacunaService.AddAsync(model);
                if (!createdItem)
                {
            ...
            {
                bool updatedItem = await _vacunaService.UpdateAsync(model);
                if (!updatedItem)
                {
        ...
        public async Task<IActionResult> Remove(int id)
        {
            ...
            {
                bool deletedItem = await _vacunaService.RemoveAsync(id);
                if (!deletedItem)
                {
